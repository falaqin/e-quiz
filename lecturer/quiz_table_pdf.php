...
$class = $_GET['class'];
$id = $_GET['id'];
$sqlMarksTable = "SELECT s.std_id, sc.std_points, sc.total_points, s.std_matric, s.std_name FROM student_quiz sq JOIN student_score sc, student s WHERE sq.quiz_id = sc.quiz_id AND s.std_id = sc.std_id AND sq.class_id = s.class_id AND sq.class_id = $class AND sq.quiz_id = $id";
